  ...
  return (
    <main className="min-h-screen bg-zinc-950 p-6 text-zinc-100">
      <div className="mx-auto max-w-md rounded-2xl border border-white/10 bg-zinc-900/60 p-6">
        ...
        <p className="mt-1 text-sm text-zinc-400">Start building ATS-ready resumes.</p>
        <form onSubmit={submit} className="mt-5 space-y-3">
          <input className="w-full rounded-lg border border-white/10 bg-zinc-800 p-2" placeholder="Full name" value={name} onChange={(e) => setName(e.target.value)} />
          ...
          <input className="w-full rounded-lg border border-white/10 bg-zinc-800 p-2" placeholder="Password (min 6 chars)" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
